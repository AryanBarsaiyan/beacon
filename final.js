        ...
        if (checkbox == true) {
          ...
          if (response.ok) {
            ...
            if (website === forMattchWebsite) {
              ...
              console.log("Airtable record updated successfully by crunchbase!");
            } else {
...
app.get('/apollo', async (req, res) => {
  try {
    ...
    await new Promise((resolve, reject) => {
      table.select({
        ...
        for (const record of records) {
          listOfRecords.push(record);
          const company_name = record.get('Company Name');
        ...
        resolve(); // Resolve the promise when all pages are fetched
        res.send(listOfRecords);
      });
    });
    ...
    console.error('Error in /apollo route:', error);
    res.status(500).send('Internal Server Error');
  }
});
